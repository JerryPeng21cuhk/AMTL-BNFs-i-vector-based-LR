...
class XconfigGRLayer(XconfigLayerBase):
    """
    This class is for lines like
     'gradient-reversal-layer name=affine input=Append(-2,-1,0,1,2,ReplaceIndex(ivector, t, 0))'
    ...
    """
    def __init__(self, first_token, key_to_value, prev_names=None):
        assert first_token == 'gradient-reversal-layer'
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def output_dim(self, auxiliary_output=None):
        # Output dim should be the same as input-dim
        return self.descriptors['input']['dim']

    def gr_weight(self):
        return self.config['gr-weight']
    ...
    def get_full_config(self):
        ...
        output_dim = self.output_dim()
        # transform_file = self.config['affine-transform-file']
        gr_weight = self.gr_weight()
        # write the 'real' component to final.config
        # line = 'component name={0} type=GradientReversalComponent dim={1} gr-weight={2}'.format(
            # self.name, input_dim, gr_weight)
        line = 'component name={0} type=GradientReversalComponent dim={1} self-repair-scale={2}'.format(
            self.name, input_dim, gr_weight)
        ans.append(('final', line))
        ans.append(('ref', line))
        # # write a random version of the component, with the same dims, to ref.config
        # line = 'component name={0} type=GradientReversalComponent input-dim={1} output-dim={2}'.format(
            # self.name, input_dim, output_dim)
        # ans.append(('ref', line))
        # the component-node gets written to final.config and ref.config.
        line = 'component-node name={0} component={0} input={1}'.format(
