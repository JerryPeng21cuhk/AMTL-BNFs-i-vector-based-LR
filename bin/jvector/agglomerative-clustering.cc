...
void AgglomerativeClusterer::MergeClusters(int32 i, int32 j) {
  ...
  switch(lkg_) {
    ...
    case lkg_complete:
      for (it = active_clusters_.begin(); it != active_clusters_.end(); ++it) {
        // The new cost is the max of the costs of the new cluster's parents
        BaseFloat new_cost = std::max(GetCost(*it, i), GetCost(*it, j));
        cluster_cost_map_[std::make_pair(*it, count_)] = new_cost;
        if (new_cost <= thresh_)
          queue_.push(std::make_pair(new_cost,
              std::make_pair(static_cast<uint16>(*it),
                             static_cast<uint16>(count_))));
      }
      break;
    case lkg_single:
      for (it = active_clusters_.begin(); it != active_clusters_.end(); ++it) {
        // The new cost is the min of the costs of the new cluster's parents
        BaseFloat new_cost = std::min(GetCost(*it, i), GetCost(*it, j));
        cluster_cost_map_[std::make_pair(*it, count_)] = new_cost;
        if (new_cost <= thresh_)
          queue_.push(std::make_pair(new_cost,
              std::make_pair(static_cast<uint16>(*it),
      ...
      break;
    default:
      KALDI_ERR << "Invalid Linkage used!";
      break;
  }
  active_clusters_.insert(count_);
...
void AgglomerativeCluster(
  ...
  AgglomerativeClusterer ac(costs, thresh, min_clust, lkg, assignments_out);
  ac.Cluster();
}
