// jvectorbin/jvector-compute-wccn.cc
...
void ComputeWhitenTransform(
    const std::map<std::string, Vector<BaseFloat> *> &utt2jvector,
    const std::map<std::string, std::vector<std::string> > &spk2utt,
    BaseFloat total_covariance_factor,
    MatrixBase<BaseFloat> *whiten_out) {
  KALDI_ASSERT(!utt2jvector.empty());
  int32 dim = whiten_out->NumCols();
  KALDI_ASSERT(dim == utt2jvector.begin()->second->Dim());
  ...
  for (iter = spk2utt.begin(); iter != spk2utt.end(); ++iter) {
    ...
    for (int32 n = 0; n < N; n++) {
      ...
      KALDI_ASSERT(utt2jvector.count(utt) != 0);
      utts_of_this_spk.Row(n).CopyFromVec(
          *(utt2jvector.find(utt)->second));
    }
  ...
  KALDI_ASSERT(!stats.SingularTotCovar() &&
               "Too little data for jVector dimension.");
  ...
  whiten_out->CopyFromMat(T);

}

void ComputeAndSubtractMean(
    std::map<std::string, Vector<BaseFloat> *> utt2jvector,
    Vector<BaseFloat> *mean_out) {
  int32 dim = utt2jvector.begin()->second->Dim();
  size_t num_jvectors = utt2jvector.size();
  Vector<double> mean(dim);
  std::map<std::string, Vector<BaseFloat> *>::iterator iter;
  for (iter = utt2jvector.begin(); iter != utt2jvector.end(); ++iter)
    mean.AddVec(1.0 / num_jvectors, *(iter->second));
  mean_out->Resize(dim);
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Whitenning jVector system.  Reads in jVectors per utterance,\n"
        "and an utt2spk file which it uses to help work out the within-speaker and\n"
        "between-speaker covariance matrices.  Outputs an WCCN projection to a\n"
        "specified dimension. \n"
        "Note: the transform we produce is actually an affine transform which will\n"
        "also set the global mean to zero.\n"
        "\n"
        "Usage:  jvector-compute-wccn [options] <jvector-rspecifier> <utt2spk-rspecifier>\n"
        " <wccn-matrix-out>"
        "e.g.: \n"
        " jvector-compute-wccn ark:jvectors.ark ark:utt2spk wccn.mat\n";
    ...
    std::string jvector_rspecifier = po.GetArg(1),
        utt2spk_rspecifier = po.GetArg(2),
        wccn_wxfilename = po.GetArg(3);

    int32 num_done = 0, num_err = 0, dim = 0;

    SequentialBaseFloatVectorReader jvector_reader(jvector_rspecifier);
    ...
    for (; !jvector_reader.Done(); jvector_reader.Next()) {
      std::string utt = jvector_reader.Key();
      const Vector<BaseFloat> &jvector = jvector_reader.Value();
      if (utt2jvector.count(utt) != 0) {
        KALDI_WARN << "Duplicate jVector found for utterance " << utt
                   << ", ignoring it.";
        num_err++;
        continue;
      }
      ...
      utt2jvector[utt] = new Vector<BaseFloat>(jvector);
      if (dim == 0) {
        dim = jvector.Dim();
      } else {
        KALDI_ASSERT(dim == jvector.Dim() && "jVector dimension mismatch");
      }
    ...
    ComputeAndSubtractMean(utt2jvector, &mean);
    // utt2jvector is centered and mean is computed.
    KALDI_LOG << "2-norm of jVector mean is " << mean.Norm(2.0);

    // Create whitening matrix and transform i-vector!!!!
    Matrix<BaseFloat> whiten_mat(dim, dim + 1);
    SubMatrix<BaseFloat> linear_part(whiten_mat, 0, dim, 0, dim);
    ComputeWhitenTransform(utt2jvector,
                            spk2utt,
                            total_covariance_factor,
                            &linear_part);
    Vector<BaseFloat> offset(dim);
    offset.AddMatVec(-1.0, linear_part, kNoTrans, mean, 0.0);
    whiten_mat.CopyColFromVec(offset, dim);

    KALDI_VLOG(2) << "2-norm of transformed jVector mean is "
                  << offset.Norm(2.0);

    WriteKaldiObject(whiten_mat, wccn_wxfilename, binary);

    KALDI_LOG << "Wrote whiten transform to "
              << PrintableWxfilename(wccn_wxfilename);
