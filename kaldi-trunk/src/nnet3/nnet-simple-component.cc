...
std::string GradientReversalComponent::Info() const {
  std::ostringstream stream;
  stream << Type() << ", dim=" << dim_ << ", gr-weight=" << self_repair_scale_;
  return stream.str();
}
// void GradientReversalComponent::Init(int32 dim, BaseFloat gr_weight)  {
  // dim_ = dim;
  // gr_weight_ = gr_weight;
  // KALDI_ASSERT(dim_ > 0 &&
               // dim_ );
// }
// void GradientReversalComponent::InitFromConfig(ConfigLine *cfl) {
  // bool ok = cfl->GetValue("dim", &dim_);
  // bool ok_gr = cfl->GetValue("gr-weight", &gr_weight_);
  // if (!ok || !ok_gr || cfl->HasUnusedValues() || dim_ <= 0) 
	  // KALDI_ERR << "Invalid GradientReversal initilizer";
  ...
  // // bool ok =  cfl->GetValue("gr-weight",&gr_weight_);
...
void GradientReversalComponent::Backprop(const std::string &debug_info,
                             ...
                             CuMatrixBase<BaseFloat> *in_deriv) const {
  in_deriv->CopyFromMat(out_deriv);
  // in_deriv->Scale(-1 * gr_weight_);
  in_deriv->Scale(-1 * self_repair_scale_);
}
